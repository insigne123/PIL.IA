...
function validateApprovalGates(
    ...
    if (unitLower.includes('m2') || unitLower === 'mÂ²') {
        // Valid Case A: Area geometry (direct or from profile)
        const isAreaMethod = calcMethod === 'direct_area' || calcMethod?.startsWith('profile_area');
        if (evidenceType === 'area' && isAreaMethod) {
                    ...
                    if (derivedResult.canDerive) {
                        ...
                        if (derivedResult.orientation === 'horizontal') {
                            ...
                            if (layerProfile && layerProfile.total_area > 0) {
                                ...
                                console.log(`  [Matcher] Blocked Length*Hz for Horizontal Item "${excelItem.description}" - no area`);
                            }
                        } else {
